...
export async function POST(req: Request) {
  ...
  if (isContactless && !autoSubmit) {
    return NextResponse.json(
      {
        error:
          "Contactless claims need auto-submit enabled (and a TfL login) so Fifteen can read the journey charge from your TfL proof. Turn this on in Settings.",
      },
      { status: 400 },
    );
  }
  ...
  return NextResponse.json(
    {
      event: { ...event, workflowRunId, status: autoSubmit ? "detected" : event.status },
      autoSubmit,
    },
    { status: 201 },
  );
